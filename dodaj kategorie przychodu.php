	...
	if(isset($_POST['category']))
	{
		...
		try
		{
			...
			else
			{
				...
				$result=$connection->query("SELECT * FROM incomes_categories WHERE income_category='{$_POST['category']}'");
				...
				if($categories==1)
				{
					...
					$result=$connection->query("SELECT * FROM users_incomes_categories WHERE user_ID={$_SESSION['user']}");
					if(!$result) throw new Exception($connection->error);
					$categories=$result->num_rows;
					//if so mark it
					if($categories==1)
				...
				{
					if($connection->query("INSERT INTO incomes_categories VALUES (NULL,'{$_POST['category']}')"))
					{	
				...
				if($categoryAssignedToUser==false)
				{
					...
					if($connection->query("INSERT INTO users_incomes_categories VALUES ({$_SESSION['user']},$categoryID)"))
					{
